...
const fadeInLeft = { initial: { opacity: 0, x: -60 }, animate: { opacity: 1, x: 0 }, transition: { duration: 0.6, ease: "easeOut" } };
const fadeInRight = { initial: { opacity: 0, x: 60 }, animate: { opacity: 1, x: 0 }, transition: { duration: 0.6, ease: "easeOut" } };
const scaleIn = { initial: { opacity: 0, scale: 0.8 }, animate: { opacity: 1, scale: 1 }, transition: { duration: 0.6, ease: "easeOut" } };
const staggerContainer = { initial: {}, animate: { transition: { staggerChildren: 0.1 } } };
const staggerItem = { initial: { opacity: 0, y: 40 }, animate: { opacity: 1, y: 0 }, transition: { duration: 0.5, ease: "easeOut" } };
const numberAnimation = { initial: { opacity: 0, scale: 0.5, rotate: -10 }, animate: { opacity: 1, scale: 1, rotate: 0 }, transition: { duration: 0.8, ease: "easeOut" } };
  ...
  return (
    <div className={`${outfit.className} w-full max-w-full px-4`}>
      {projects.map((project, index) => {
        ...
        const portfolioImages = Array.isArray(project.portfolioImages) ? project.portfolioImages : [];

        return (
          <div key={project._id}>
            <motion.div
              className={`max-w-7xl mx-auto ${isEven ? 'bg-gray-100 p-4 rounded-2xl my-6' : 'py-3'} my-6`}
            ...
            >
              <motion.div
                className={`flex flex-col ${isEven ? 'lg:flex-row-reverse' : 'lg:flex-row'} items-start gap-8 mb-12`}
                initial="initial"
                whileInView="animate"
                viewport={{ once: true, amount: 0.3 }}
              >
                <motion.div className='flex-1' variants={isEven ? fadeInRight : fadeInLeft}>
                  <div className='flex items-center gap-6 mb-6'>
                    ...
                    <motion.div className='flex-1 w-full max-w-lg' variants={fadeInUp}>
                      <h2 className='text-2xl md:text-4xl font-semibold text-black'>
                        {project.title}
                      </h2>
                    </motion.div>
                  </div>
                  <motion.div className='pl-6 w-full max-w-xl' variants={fadeInUp}>
